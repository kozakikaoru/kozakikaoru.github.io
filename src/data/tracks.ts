// AI 楽曲のトラック定義。並びは制作順(古い→新しい)= 素材フォルダの 00〜09 に対応。
// 曲を追加するとき: public/audio/ に mp3 を置き、src/assets/music/ に 640x640 の webp を
// 置いて、下の TRACKS に 1 件足す(id/title/sub/date/durationLabel/src/lyrics/art)。
// - lyrics は空行でブロック区切り。行末の空白は入れない(取り込み時に除去済み)。
// - art.image があれば CD 盤面に画像を貼る。c1/c2/initials は画像が出ない時のフォールバック、
//   glow はプレイリストの再生中行の発光に使う(画像があっても効く)。
// - desc は現在どの画面でも描画していない(将来の紹介文用に任意で持つ)。
