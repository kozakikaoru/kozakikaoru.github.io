...
export default function Career() {
  ...
  // 年月ラベルの間引き: 直前の項目の下端(開始/日付)と、この項目の終了年月を比べ、
  // 近ければこの項目の「終了」ラベルを隠して上側の1つにまとめる。
  //   - 同月(差0)なら常に集約(例: 退社2021.04 と重なる 自動車フリマ終了2021.04)。
  //   - 隣接月(差1)は、上が「期間(案件/学校)」のときだけ集約する。
  //     連続した在籍/参画の継ぎ目だから(例: 高校卒業2014.03 は 専門入学2014.04 の
  //     下なので隠して4月だけに)。
  //   - 上が「単発イベント(節目)」なら、差1では隠さない。別の出来事で時系列が
  //     崩れるため(例: 専門卒業2016.03 は 入社2016.04[節目]の下でも残す)。
  //   - 2ヶ月以上空けば常に両方残す(例: 入社2016.04 と レシート開始2016.09)。
  const hideEnd = useMemo<boolean[]>(
    () =>
      entries.map((e, i) => {
        ...
        const gap = monthGap(bottomDate(above), end);
        if (gap == null || gap > 1) return false;
        return gap === 0 || above.kind !== 'milestone';
      }),
    [entries],
  );
